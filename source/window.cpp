...
WindowHandle CreateWindow(std::string title, int width, int height) {
  internal::nextHandle++;
  internal::windows[internal::nextHandle] = {
      .title = title, .width = width, .height = height, .visible = true};
  ...
  windowData.NativeWindowPointer =
...
void SetWindowImGuiRenderCallback(WindowHandle handle,
